// src/components/Recom_Book.js
import React, { useState } from "react";
import { issueABook } from "../actions/Issue_action";
import { useDispatch, useSelector } from 'react-redux';
import { Toast } from "react-bootstrap";
import { setMessage } from "../actions/messageActions"; // Import the setMessage action
...
const Recom_Book = () => {
    ...
    const PostData = () => {
        ...
        dispatch(setMessage(`A new book titled "${title}" has been recommended.`)); // Dispatch the message action
        setShow(true);
        setAuthor("");
        setTitle("");
        setPublisher("");
            ...
            <div className="card col-md-7 m-auto p-5">
                <Toast onClose={() => setShow(false)} show={show} delay={3000} autohide>
                    <Toast.Body style={{ backgroundColor: "green", color: "white", fontSize: "18px" }}>
                        You successfully recommended a book!
                    </Toast.Body>
                </Toast>
                <h3 style={{ textAlign: "center", fontFamily: "Oswald", fontSize: "40px" }}>
                    Recommend a Book
                </h3>
